...
RUN wget https://github.com/swagger-api/swagger-editor/archive/v3.2.4.tar.gz -O /tmp/swagger_editor.tar.gz
RUN tar -xvf /tmp/swagger_editor.tar.gz --directory /tmp
RUN mkdir /swagger_editor
RUN cp -R /tmp/swagger-editor-3.2.4/dist /swagger_editor/dist/
